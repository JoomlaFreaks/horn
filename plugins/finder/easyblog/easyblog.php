...
class plgFinderEasyBlog extends FinderIndexerAdapter
{
	...
	public function onFinderAfterSave($context, $post, $isNew)
	{
		...
		} else if ($context == 'easyblog.blog' && $post->isTrashed()) {
	...
	protected function index(FinderIndexerResult $item, $format = 'html')
	{
		...
		// if id is null or empty, stop the indexing.
		if (! $item->id) {
			return;
		}
		...
		// Get the subdomain name e.g. http://site.com/subdomain
		$subfolder = JURI::root(true);

		// Double check the current domain name is it include subdomain
        if (!empty($subfolder)) {
            $item->route = $subfolder . $item->route;
        }

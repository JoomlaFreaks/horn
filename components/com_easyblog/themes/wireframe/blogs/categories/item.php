	...
	<div class="eb-category-profile">
		...
		<div class="col-cell eb-category-thumb">
			<a href="<?php echo $category->getPermalink();?>" class="eb-avatar">
				<img src="<?php echo $category->getAvatar();?>" width="60" height="60" alt="<?php echo $category->getTitle();?>" />
			</a>
		</div>
		<?php } ?>
			...
			<div class="eb-category-subscribe spans-seperator">
				<?php if ($this->params->get('category_subscribe_email', true) && $this->config->get('main_categorysubscription')) { ?>
					<span class="eb-category-subscription">
						<a href="javascript:void(0);" class="link-subscribe <?php echo $isCategorySubscribed ? 'hide' : ''; ?>" data-blog-subscribe data-type="category" data-id="<?php echo $category->id;?>">
							<?php echo JText::_('COM_EASYBLOG_SUBSCRIPTION_SUBSCRIBE_CATEGORY'); ?>
						</a>
						<a href="javascript:void(0);" class="link-subscribe <?php echo $isCategorySubscribed ? '' : 'hide'; ?>" data-blog-unsubscribe data-type="category" data-subscription-id="<?php echo $subscriptionId;?>" data-return="<?php echo base64_encode(JRequest::getURI());?>">

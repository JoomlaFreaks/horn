...
class EasyBlogStylesheet extends EasyBlog
{
	...
	public function __construct($location, $theme, $useOverride)
	{
		...
		if ($this->location == 'composer' || $this->location == 'module') {
	...
	public function folder($name = 'current')
	{
		...
		if (!isset($cache[$name]) || defined('EASYBLOG_COMPONENT_CLI')) {
			...
			$base = $this->location == 'composer' || $this->location == 'dashboard' || $this->location == 'site' || $this->location == 'module' ? JPATH_ROOT : JPATH_ADMINISTRATOR;
			...
			if ($name == 'composer' || $name == 'dashboard' || $name == 'module' || $name == 'site' || $name == 'site_base' || $name == 'admin' || $name == 'admin_base') {

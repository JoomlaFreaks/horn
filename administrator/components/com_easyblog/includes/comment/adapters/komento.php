...
class EasyBlogCommentKomento extends EasyBlogCommentBase
{
	...
	{
		if (JString::strpos($content, '{KomentoDisable}') !== false) {
			$post->komentodisable = true;
			$content = JString::str_ireplace('{KomentoDisable}', '', $content);
		}

		if (JString::strpos($content, '{KomentoEnable}') !== false) {
			$post->komentoenable = true;
			$content = JString::str_ireplace('{KomentoEnable}', '', $content);
		}

		if (JString::strpos($content, '{KomentoLock}') !== false) {
			$post->komentolock = true;
			$content = JString::str_ireplace('{KomentoLock}', '', $content);
		}

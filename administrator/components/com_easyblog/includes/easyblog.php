...
class EB
{
	...
	{
		return EB::getJoomlaVersion() >= '3.1';
	}
	...
	public static function getMenuParams($id, $type, $menuParams = false, $debug = false)
	{
		...
		if (!isset($items[$id])) {
			...
			if ($type == 'categories') {
				...
				$categoriesParams = $model->getDefaultCategoriesXMLParams();
				$arrTmpParams = $categoriesParams->toArray();

				// If menu item is exists, lets get the params from there
				if ($menuId) {
					...
					foreach ($arrTmpParams as $key => $val) {
						$categoriesParams->set($key, $config->get($type . '_' . $key));
					}

					$items[$id] = $categoriesParams;
				}
			...
			// If menuParams already being passed, we just use it without load another menu
			if ($menuParams) {
				$params = $menuParams;
				$arrParams = $params->toArray();

				// We need to merge the global view settings for tag and category layout
				if ($type == 'category' || $type == 'tag' || $type == 'blogger') {
					$arrParams = array_merge($arrTmpParams, $arrParams);
				}

			} else {
				// Here we try to get the menu id for based on id passed.
				if ($type == 'category') {
				...
				// if the menu id is exist, get the params
				if ($menuId) {
					$params = $model->getMenuParamsById($menuId);
					$arrParams = $params->toArray();
				}
			...
			if ($arrParams) {
				...
				// Now we get the params already, we need to process it

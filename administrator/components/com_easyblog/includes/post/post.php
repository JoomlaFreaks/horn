...
class EasyBlogPost extends EasyBlog
{
	...
	public function bindParams($newParams)
	{
		...
		if ($newParams) {
			...
			$registry = new JRegistry($newParams);

			$params->merge($registry);

			// Reset acl notifications
			if (!$registry->get('aclNotification')) {
				$params->set('aclNotification', '');
			}
		}
	...
	public function trash()
	{
		...
		$options = array('validateData' => false, 'normalizeData' => false, 'processAutopost' => false, 'skipCategoriesUpdate' => true, 'skipTagsUpdate' => true);
	...
	public function archive()
	{
		...
		$options = array('validateData' => false, 'normalizeData' => false, 'skipCategoriesUpdate' => true, 'skipTagsUpdate' => true);
		return $this->save($options);
	}
	...
	public function duplicate()
	{
		...
		$state = $post->save(array('applyDateOffset' => true, 'normalizeData' => false, 'copyPost' =>true));
	...
	public function move($categoryId)
	{
		...
		$options = array_merge(array(), array('validateData' => false, 'normalizeData' => false, 'skipTagsUpdate' => true, 'processAutopost' => false));
	...
	public function approve()
	{
		...
		$this->save(array('validateData' => false, 'skipCategoriesUpdate' => true, 'skipTagsUpdate' => true));
	}
	...
	public function publish($options = array(), $forceNotNew = false)
	{
		...
		// Set the save options
		$options = array_merge(array(), array('validateData' => false, 'normalizeData' => false, 'skipCategoriesUpdate' => true, 'skipTagsUpdate' => true), $options);
	...
	public function unpublish()
	{
		...
		$options = array('validateData' => false, 'skipCategoriesUpdate' => true, 'skipTagsUpdate' => true);
	...
	public function extractLegacyMedia()
	{
		...
		if (isset($this->posttype) && $this->posttype == 'standard' || !$this->posttype) {
	...
	public function postSave()
	{
		...
		if ($this->saveOptions['processAutopost']) {
	...
	public function toEmailData()
	{
		...
		$data['blogIntro'] = preg_replace('/(<[^>]+) style=".*?"/i', '$1', $data['blogIntro']);
		$data['blogContent'] = preg_replace('/(<[^>]+) style=".*?"/i', '$1', $data['blogContent']);
	...
	public function getPreviewLink($xhtml = true)
	{
		...
		if ($this->app->isAdmin() && $sh404exists) {
			...
			$url = Sh404sefHelperGeneral::getSefFromNonSef($url, true, true);
	...
	public function getExternalPermalink()
	{
		...
		if (!isset($link[$id])) {
			...
			// If this is being submitted from the back end and sh404sef is enabled we will generate link based on sh404sef
			if ($app->isAdmin() && $sh404) {
				$link[$id] = Sh404sefHelperGeneral::getSefFromNonSef($url, true, true);

				// We need to remove the /administrator/ from the link
				$link[$id] = str_ireplace('/administrator/', '/', $link[$id]);
			}
		}
	...
	public function hasReadmore()
	{
		...
		$introlength	= JString::strlen(strip_tags($this->intro));
		...
		$fulllength	= JString::strlen(strip_tags($this->intro . $this->content));
	...
	public function renderHeaders($options = array())
	{
		...
		// Get the page title
		$title = EB::getPageTitle(JText::_($this->config->get('main_title')));
		...
		if ($this->config->get('main_pagetitle_autoappend_entry') && $title) {
			...
			if ($jPageTitleFormat == 1) {
				$this->doc->setTitle($title . ' - ' . $postTitle);
			}
		}

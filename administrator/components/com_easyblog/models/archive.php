...
class EasyBlogModelArchive extends EasyBlogAdminModel
{
	...
	public function getArchivePostCounts($yearStart='', $yearStop='0', $excludeCats = '', $includeCats = '', $filter = '', $filterId = '', $filterType = 'normal')
	{
		...
		if ($easysocial->exists() && !EB::isSiteAdmin()) {
	 ...
	 * @return
	 */
	public function getArchivePostListByMonth($month='', $year='', $showPrivate = false, $category = '', $filterType = 'normal')
	{
		$db = EB::db();
		$user = JFactory::getUser();
		$config = EB::config();
		...
		if (EB::easysocial()->exists() && !EB::isSiteAdmin()) {

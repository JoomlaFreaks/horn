...
class EasyBlogModelBlog extends EasyBlogAdminModel
{
	...
	public function getBlogComment($id, $limitFrontEnd = 0, $sort = 'asc', $lite = false)
	{
		...
		$limit = ($app->getCfg('list_limit') == 0) ? 5 : $app->getCfg('list_limit');
	...
	*/
								...
								$options = array())
	{
		...
		if (JRequest::getCmd('format', '') == 'feed' && $config->get('main_password_protect')) {
		...
		if ($easysocial->exists() && !EB::isSiteAdmin() && $type != 'teamblog' && !$dashboard) {
		...
		if ($fieldsFilter) {
			...
			foreach ($fieldsFilter as $fieldId => $values) {
				...
				$fieldQ = 'select distinct `post_id` from `#__easyblog_fields_values` WHERE ';
				$fieldQ .= '`field_id` = ' . $db->Quote($fieldId) . ' AND (';
	 ...
	 * @access	public
	 */
	public function getTaggedBlogs($tagId = 0, $limit = false, $includeCatIds = '', $sorting = '', $excludeFeatured = false)
	{
		if (!$tagId) {
			return false;
		}
	...
	public function getRelatedPosts($id, $max = 0)
	{
		...
		// category access here
		$catLib = EB::category();
		$catAccessSQL = $catLib->genAccessSQL('c.`id`', array());
		$query[] = ' AND (' . $catAccessSQL . ')';
